...
mod_08_cty_mean <- mpg %>% filter(year == 2008) %>%
    group_by(model) %>% summarise(City= mean(cty)) %>% arrange(desc(City)) %>%   head(3)
mod_99_cty_mean <- mpg %>% filter(year == 1999) %>%
    group_by(model) %>% summarise(City= mean(cty)) %>% arrange(desc(City)) %>%   head(3)
mod_08_hwy_mean <- mpg %>% filter(year == 2008) %>%
    group_by(model) %>% summarise(City= mean(hwy)) %>% arrange(desc(City)) %>%   head(3)
mod_99_hwy_mean <- mpg %>% filter(year == 1999) %>%
    group_by(model) %>% summarise(City= mean(hwy)) %>% arrange(desc(City)) %>%   head(3)
table2 <-cbind(mod_99_cty_mean, mod_99_hwy_mean,mod_08_cty_mean,mod_08_hwy_mean)

colnames(table2) <- c('Model', 'Milage', 
                     "Model", "Milage",
                     'Model', 'Milage',
                     "Model", "Milage"
                     )
kable(table2, digits = 2, format = "latex", booktabs=TRUE, ,caption = "Top 3 MPG Performing Cars: 1999, 2008") %>% kable_styling(latex_options = 'hold_position',font_size = 12,full_width = T,position = "center") %>%
